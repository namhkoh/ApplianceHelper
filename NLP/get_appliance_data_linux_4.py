...
def boi_save_train_test(target, slots, slot_dict_boi, intent_dict, single=False):
	...
	with open('./data/appliance/train', 'w') as f_train, open('./data/appliance/test', 'w') as f_test, open(
			'./data/appliance/valid', 'w') as f_valid:
		# Intent number
		i = 0
		for s in range(start, end):
			...
			for j in range(len(target[s])):
				...
				if len(slots[s]) == 2:
					p: int
					line, p = boi_two_slots_write(s, j, f_train, f_test, f_valid, slots, slot_dict_boi, target,
												  intent_dict)
					i += p
				elif len(slots[s]) == 3:
					line, _ = boi_three_slots_write(s, j, f_train, f_test, f_valid, slots, slot_dict_boi, target,
													intent_dict)
				elif len(slots[s]) == 1:
					line, _ = boi_one_slot_write(s, j, f_train, f_test, f_valid, slots, slot_dict_boi, target,
												 intent_dict)
			# print("-----------------------------------------------")
			# print("-----------------------------------------------")
		...
		f_train = open('./data/appliance/train', 'r')
		f_test = open('./data/appliance/test', 'r')
		f_valid = open('./data/appliance/valid', 'r')

		print(len(f_train.readlines()))
		print(len(f_test.readlines()))
		print(len(f_valid.readlines()))
