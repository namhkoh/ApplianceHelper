...
def boi_save_train_test(target, slots, slot_dict_boi, intent_dict, single = False):
	...
	for s in range(start, end):
		...
		for j in range(len(target[s])):
			if len(slots[s]) == 2:
				line, _ = boi_two_slots_write(s, j, f, slots, slot_dict_boi, target, intent_dict)
			elif len(slots[s]) == 3:
				line, _ = boi_three_slots_write(s, j, f, slots, slot_dict_boi, target, intent_dict)
			elif len(slots[s]) == 1:
				line, _ = boi_one_slot_write(s, j, f, slots, slot_dict_boi, target, intent_dict)
				# print("-----------------------------------------------")
				# print("-----------------------------------------------")
	f.close()


def boi_one_slot_write(s, j, f, slots, slot_dict_boi, target, intent_dict):
	p = 0
	# Slot 1
...
def boi_two_slots_write(s, j, f, slots, slot_dict_boi, target, intent_dict):
	p = 0
	x_boi = ""
	# Slot 1
	for i in range(len(slot_dict_boi[slots[s][0]])):
...
def boi_three_slots_write(s, j, f, slots, slot_dict_boi, target, intent_dict):
	p = 0
	# Slot 1
	for i in range(len(slot_dict_boi[slots[s][0]])):
		# Slot 2
...
def boi_save_train_test_valid():
	my_file = open('./data/appliance/train_test', 'r')
	...
	f = open('./data/appliance/train', 'w')
	for i in train:
		f.write(i)

	f = open('./data/appliance/test', 'w')
	for i in test:
		f.write(i)
